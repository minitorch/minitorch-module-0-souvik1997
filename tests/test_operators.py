...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    sigmoid_a = sigmoid(a)
    assert (lt(sigmoid_a, 1.0) or eq(sigmoid_a, 1.0)) and (lt(0.0, sigmoid_a) or eq(0.0, sigmoid_a))
    assert_close(add(1, neg(sigmoid_a)), sigmoid(neg(a)))
    assert eq(sigmoid(0), 0.5)
    if eq(1, sigmoid(a - 1)):
        assert eq(1, sigmoid_a)
    else:
        assert lt(sigmoid(a - 1), sigmoid_a) or eq(sigmoid(a - 1), sigmoid_a)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert_close(add(sum(ls1), sum(ls2)), sum(zipWith(lambda x, y: add(x, y), ls1, ls2)))
